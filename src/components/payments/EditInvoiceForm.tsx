		...
		try {
			...
			if (invoice.stripe_invoice_id) {
				try {
					...
					if (response.error) {
                     ...
                     Please contact support for assistance. Error: ${
						...
						if (response.data && response.data.stripe_invoice_id) {
							...
							if (updateError) {
								console.error(
									"Error updating invoice with new Stripe details:",
									updateError
								);
							}
						}
                   ...
                   The changes may not be reflected in the payment system. Error: ${
			...
			if (invoice.status === "pending" && profile?.studio?.id) {
				try {
					...
					let parentId = invoice.parent_id;
					if (!parentId) {
						const { data: invoiceData, error: parentLookupError } =
							await supabase
								.from("invoices")
								.select("parent_id")
								.eq("id", invoice.id)
						...
						{recurringInterval === "month" && (
							<div>
								...
								<p className="mt-1 text-sm text-gray-500">
									Note: Payment will be processed on the last day of the month
